        ...
        <div class="d-flex align-items-center">
            ...
            <div>
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="<?php echo base_url('scheduling_em') ?>"><i class="ti ti-arrow-left"></i></a></li>
    ...
    <div class="col-md-12 col-lg-12">
        <div class="widget has-shadow">            
            <div class="widget-body">
                <?php echo form_open('subform/submit_risk/'.$pra->id.'/'.$id); ?>
                <div class="form-group row">
                    <label class="form-control-label col-lg-2">Pilot / Test Lot</label>
                    <div class="col-lg-5">
                        <?php if ($this->session->userdata('department') == "Technical") { ?>
                            <input type="text" placeholder="Pilot / Test Lot" name="add" required="" value="<?php echo $pra->pilot ?>" class="form-control">
                        <?php }else{ ?>
                            <input type="text" placeholder="Pilot / Test Lot" value="<?php echo $pra->pilot ?>" class="form-control" readonly>                            
                        <?php } ?>
                    </div>
...
<div class="row">
    <div class="col-lg-12 text-right">
        ...
        <?php }else{ ?>
            <a href="#" onclick="window.history.go(-1); return false;" class="btn btn-primary btn-sm">Back</a>
        <?php } ?>
    </div>
    <?php echo form_close(); ?>
</div>
...
<div id="modal-centered2" class="modal fade" style="display: none;" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
            <?php echo form_open_multipart('subform/add_detail_risk/'.$id); ?>
                <div class="mt-5 mb-5 position-relative">
                    <div class="group material-input">
...
<script>
    $(document).ready(function(){
        ...
        function view_risk() {
            var id = '<?php echo $id ?>';
            $.ajax({
                url : "<?php echo site_url('index.php/subform/get_risk')?>/"+id,
                type: "GET",
                dataType: "JSON",
        ...
        function view_detail() {
            var id = '<?php echo $id ?>';
            $.ajax({
                url : "<?php echo site_url('index.php/subform/get_risk_detail')?>/"+id,
                type: "GET",
                dataType: "JSON",
                success: function(data){
                    var html = '';
                    ...
                    for(i=0; i<data.length; i++){   
                        html += "<tr class='text-center'>"+
                            ...
                            "<td><img src='<?php echo base_url() ?>"+data[i].photo_1+"' height='60'></td>"+
                            "<td>"+data[i].field_6+"</td>"+
                            "<td><img src='<?php echo base_url() ?>"+data[i].photo_2+"' height='60'></td>"+
        ...
        $(".add-risk").click(function(){
            var id = '<?php echo $id ?>';             
            url = "<?php echo site_url('index.php/subform/add_risk/')?>"+id;                              
              $.ajax({
                url : url,
                type: "POST",
                data: $('#form').serialize(),            
        ...
        $('#show_data').on('click','.delete-risk',function(){
            ...
            if (result) {                   
                $.ajax({
                    type : "POST",
                    url  : "<?php echo base_url('index.php/subform/delete_risk')?>",
                    dataType : "JSON",
                        data : {id: id},
                        success: function(data){                        
                            view_risk();
        ...
        $('#show_data2').on('click','.delete-detail',function(){
            ...
            if (result) {                        
                $.ajax({
                    type : "POST",
                    url  : "<?php echo base_url('index.php/subform/delete_risk_detail')?>",
                    dataType : "JSON",
                        data : {id: id},
                        success: function(data){                        
                            view_detail();
        ...
        $('#submit').submit(function(e){      
            ...
            e.preventDefault(); 
                $.ajax({
                    url:'<?php echo base_url();?>index.php/subform/add_detail_risk/'+id,
                    type:"post",
